...
messages = [{"role":"system","content":"你是金融专家，将文本分类为['新闻报道', '财务报告', '公司公告', '分析师报告']，不清楚的分类为'不清楚类别' 下面有示例："}]
...
for q in questions:
    response = client.chat.completions.create(
        model="qwen-plus",
        #f是格式化字符串，不加会原样输出。类似于js中的``里面要加$符号，在python中要加f符号
        messages=messages+[{"role":"user","content":f"按照案例回答这段文本的分类类别：{q}"}]
    )
